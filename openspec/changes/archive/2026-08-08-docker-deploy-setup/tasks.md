...
- [x] **5.3** [VERIFY] Load `http://localhost:3000` in an actual browser and
  ...
  Exit criterion: 3.
  **FULLY VERIFIED**: user logged in with a real Supabase account, login
  succeeded, proving the bundle contains the actual Supabase credentials.
- [x] **5.4** [VERIFY] Confirm the server-side onboarding proxy reaches the
  ...
  **not** evidence — `fetchProfileStatus` fails open by design, so a silently
  ...
  Exit criterion: 4.
  **FULLY VERIFIED**: user logged in with a real Supabase account with an
  incomplete profile — the proxy redirected to `/onboarding` as expected.
  The literal redirect decision fired correctly. `export const runtime =
  "nodejs"` was not needed.
- [x] **5.5** [VERIFY] `docker compose -f docker-compose.yml -f
  ...
  it pre-emptively.
  Exit criterion: 5.
  **FULLY VERIFIED**: dev overlay tested with both backend and frontend hot
  reload working. Two bugs were found and fixed during this verification:
  (1) D5/D12 interaction — dev overlay's backend (no HEALTHCHECK) vs base
  file's frontend (depends_on: condition: service_healthy) — fixed with
  explicit override in dev compose. (2) Dev/prod image-tag collision — fixed
  with explicit `image:` tags in dev overlay.
- [x] **5.6** [VERIFY] Confirm the `docker-compose.dev.yml` volume merge
  ...
  Exit criterion: 5.
  **VERIFIED**: both mounts active simultaneously with no conflict, confirmed
  via `docker inspect`.
